"""Select a prosodically varied subset of the filtered YODAS2-Sidon CSV.

    python scripts/select/yodas2_sidon.py --min-per-speaker 3 --target-per-speaker 7

Takes the output of scripts/filter/yodas2_sidon.py — or better, of
scripts/filter/yodas2_sidon_speaker_fixup.py — as the input pool, scores every clip with
...
A speaker here is a video, not a person, unless the fixup script has already merged the videos
that share one. Run it first: without it the per-speaker bounds act on a label that repeats the
same host under dozens of IDs, which is what those bounds exist to prevent.

Male voices outnumber female ones in this corpus as in the others, so speakers above PITCH_EDGE
get their own --high-target-per-speaker / --high-max-per-speaker instead, contributing more clips
each. The edge is measured from the audio: YODAS2-Sidon ships no gender label.
"""
...
from scripts.filter.yodas2_sidon import AUDIO, DATASET, SEPARATOR
...
def read_pool(path):
    ...
    if not path.exists():
        raise SystemExit(f"No input pool at {path}; run scripts/filter/yodas2_sidon.py first")
