"""Select a prosodically varied subset of the filtered WolneLektury CSV.

    python scripts/select/wolne_lektury.py --min-per-speaker 3 --target-per-speaker 7

Takes the output of scripts/filter/wolne_lektury.py as the input pool, scores every clip with
...
front. A speaker here is one audiobook, not a person, so a prolific narrator appears under
several of them.

Male voices outnumber female ones roughly 3:1 here, so speakers above PITCH_EDGE get their own
--high-target-per-speaker / --high-max-per-speaker instead, contributing more clips each. The
corpus does carry a gender column, but the edge is measured from the audio as it is for Emilia,
so the two corpora are selected on the same footing.

The audio lives in the parquet shards rather than on disk, so each prosody pass is a scan of
--root instead of a seek per clip; see score().
"""
...
def read_pool(path):
    ...
    if not path.exists():
        raise SystemExit(f"No input pool at {path}; run scripts/filter/wolne_lektury.py first")
