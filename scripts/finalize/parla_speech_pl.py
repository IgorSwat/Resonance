"""Encode the selected ParlaSpeech-PL clips to Higgs audio tokens.

    python scripts/finalize/parla_speech_pl.py --input parla_speech_pl_selected.csv

Takes the output of scripts/select/parla_speech_pl.py and materialises the subset: the wav next
to the tokens, one file each per clip. The clips are OmniVoice's own 24 kHz output, which is
already the rate the codec wants, so nothing is resampled in practice.

Unlike the Emilia finalizer there is no separate corpus to copy out of — the filter generated
these clips, so --audio-dir defaults to where they already sit and the audio stays put. Point it
somewhere else and each selected clip is *moved* there, which also leaves the unselected
regenerations behind in --root for deletion.
"""
...
from scripts.select.parla_speech_pl import DATASET, ROOT, locate, read_pool
from tools.codec.higgs import HiggsCodec, to_codec_rate

PROCESSED = pathlib.Path("data/processed/ParlaSpeech-PL")
